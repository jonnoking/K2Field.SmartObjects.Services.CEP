    ...
    $.hubConnection.prototype.createHubProxies = function () {
        ...
        proxies['cEPHub'].server = {
            ...
            sendEvent: function (ev) {
                return proxies['cEPHub'].invoke.apply(proxies['cEPHub'], $.merge(["SendEvent"], $.makeArray(arguments)));
            }
        };
...
(function ($) {
    //TODO: if necessary, add additional statements to initialize each part of the namespace before your CEPEventReceiveControl is called. 
    ...
    if (typeof K2Field.SmartForms.CEPEvent === "undefined" || K2Field.SmartForms.CEPEvent == null) K2Field.SmartForms.CEPEvent = {};

    K2Field.SmartForms.CEPEvent.CEPEventReceiveControl = {
        ...
        _getInstance: function (id) {
            ...
            if (control.length == 0) {
                throw 'CEPEventReceiveControl \'' + id + '\' not found';
            } else {
                return control[0];
            }
        },
        ...
        getValue: function (objInfo) {
            ...
            var instance = K2Field.SmartForms.CEPEvent.CEPEventReceiveControl._getInstance(objInfo.CurrentControlId);
        ...
        getProperty: function (objInfo) {
            ...
            if (objInfo.property.toLowerCase() == "value") {
                return K2Field.SmartForms.CEPEvent.CEPEventReceiveControl.getValue(objInfo);
            }
            else {
        ...
        setProperty: function (objInfo) {
            switch (objInfo.property.toLowerCase()) {
                case "style":
                    K2Field.SmartForms.CEPEvent.CEPEventReceiveControl.setStyles(null, objInfo.Value, $('#' + objInfo.CurrentControlId));
                    break;
                case "value":
                    K2Field.SmartForms.CEPEvent.CEPEventReceiveControl.setValue(objInfo);
                    break;
                case "isvisible":
                    K2Field.SmartForms.CEPEvent.CEPEventReceiveControl.setIsVisible(objInfo);
                    break;
                case "isenabled":
                    K2Field.SmartForms.CEPEvent.CEPEventReceiveControl.setIsEnabled(objInfo);
                    break;
                default:
        ...
        setStyles: function (wrapper, styles, target) {
            ...
            var options = {};
            var element = isRuntime ? jQuery(target) : wrapper.find('.K2Field.SmartForms.CEPEvent.CEPEventReceiveControl');
...
$(document).ready(function () {
    ...
    var id = $(".K2Field-SmartForms-CEPEvent-CEPEventReceiveControl-Control").attr("Id");
    //$("#" + id).attr("value", d.message);

    $.connection.hub.url = $("#" + id).attr("data-huburl");

    var cep = $.connection.cEPHub;

    cep.client.sendEvent = function (ev) {

        var fltr = "";
        var fa = [];
        if ($("#" + id).attr("data-eventfilter") != undefined) {
            fltr = $("#" + id).attr("data-eventfilter");
            var fa = fltr.split("|");
            for (var i = 0; i < fa.length; i++) {
                fa[i] = fa[i].toLowerCase();
            }
        }
        ...
        if (ev.hasOwnProperty("EventType"))
        {
            ...
            //{
                ...
                if (ev.hasOwnProperty("EventSource")) {
                    $("#" + id).attr("data-eventsource", ev.EventSource);
                }
                if (ev.hasOwnProperty("Action")) {
                    $("#" + id).attr("data-action", ev.Action);
                }
                if (ev.hasOwnProperty("ProcessName")) {
                    $("#" + id).attr("data-processname", ev.ProcessName);
                }
                if (ev.hasOwnProperty("ProcessId")) {
                    $("#" + id).attr("data-processid", ev.ProcessId);
                }
                if (ev.hasOwnProperty("EventData")) {
    ...
    cep.client.send = function (name, message, msgdatetime) {
        ...
        $("#" + id).attr("data-eventdate", msgdatetime);
    };
    ...
    $.connection.hub.stateChanged(function (change) {     
        if (change.newState === $.signalR.connectionState.connecting) {
            $("#" + id).attr("data-connectionstate", "Connecting");
            raiseEvent(id, 'Control', 'HubConnecting');            
        }
        else if (change.newState === $.signalR.connectionState.connected) {
            $("#" + id).attr("data-connectionstate", "Connected");
            raiseEvent(id, 'Control', 'HubConnected');
        }
        else if (change.newState === $.signalR.connectionState.reconnecting) {
            $("#" + id).attr("data-connectionstate", "Reconnecting");
            raiseEvent(id, 'Control', 'HubReconnecting');
        }
        else if (change.newState === $.signalR.connectionState.disconnect) {
            $("#" + id).attr("data-connectionstate", "Disconnected");
            raiseEvent(id, 'Control', 'HubDisconnected');
        }
